...
class DetailNonAkademik extends StatelessWidget {
  DetailNonAkademik(
      this.namaKelas,
      this.kegiatanId,
      this.namaKegiatan,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        child: SafeArea(
            child: FutureBuilder(
          future: PimpinanController()
              .getDetailNonAkademik(tanggal, hari, kelasId, kegiatanId),
          builder: (context, AsyncSnapshot snapshot) {
            ...
            if (snapshot.hasData) {
              ...
              var narasumber;
              var topik;
              if (data['jadwal']['kegiatan']['narasumber'] == '0') {
                kegiatan = data['jadwal']['monitoring_kegiatan'];
                narasumber = '-';
                topik = '-';
                print(kegiatan);
              } else {
                kegiatan = data['jadwal']['monitoring_kegnas'];
                print('aaaaaaaaaa');
                if (kegiatan.length != 0) {
                  narasumber = kegiatan[0]['narasumber']['nama'];
                  topik = kegiatan[0]['topik'];
                } else {
                  narasumber = '-';
                  topik = '-';
                }
              ...
              var waktuMulaiKegiatan = '';
              var waktuBerakhirKegiatan = '';
              var presensi = data['presensi'];
              if (kegiatan.length != 0) {
                waktuMulaiKegiatan =
                    kegiatan[0]['waktu_mulai'].toString().substring(0, 5);
                waktuBerakhirKegiatan =
                    kegiatan[0]['waktu_berakhir'].toString().substring(0, 5);
                return Column(
                  children: [
                    Container(
                        // height: heightApp * 0.05,
                    ...
                    Container(
                      ...
                      child: Text('${tanggal}'
                          ' | '
                          '${waktuMulaiKegiatan} WIB'
                          ' - '
                          '${waktuBerakhirKegiatan} WIB'),
                    ),
                    Container(
                        // height: heightApp * 0.05,
                        padding: EdgeInsets.symmetric(
